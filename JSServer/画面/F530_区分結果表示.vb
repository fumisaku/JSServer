Public Class F530_区分結果表示
    ...
    Private Sub F530_区分結果表示_Load(sender As Object, e As EventArgs) Handles MyBase.Load

    End Sub


    Public Sub New(区分番号_ As String)
    ...
    Private Sub DGV更新()
            ...
            If SC_J_区分結果.SC_J_区分結果_選手(s).No <> 0 Then
                ...
                For rr = 1 To UBound(対象区分番号)
                    If 対象区分_NO(rr) <> 0 Then
                        tbl.Rows(idx).Item(7 + rr) = 変換(SC_J_区分結果, rr, s)
                    End If
                Next rr
        ...
        For r = 1 To 19
            If 対象区分_NO(r) <> 0 Then
                Me.DGV_区分結果.Columns(7 + r).Visible = True
            Else
                Me.DGV_区分結果.Columns(7 + r).Visible = False
            End If
        Next r
