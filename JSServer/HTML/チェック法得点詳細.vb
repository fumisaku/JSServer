Public Class チェック法得点詳細


    'チェック法の得点詳細をHTMLで書き出す
    ...
    Public Sub CreateHTML(採点結果 As 採点結果_C, LOCAL指定 As String)
        ...
        For 順位 = 1 To 採点結果.出場選手数
            For s = 1 To 採点結果.出場選手数
                If 採点結果.総合順位表記(s) = 順位 Then
                    ...
                    i = i + 1 : データ(i) = "         <td rowspan=""2"" class=""Rank""  >" & 採点結果.総合得点(s).ToString("0") & "</td>"
                    For d = 1 To 採点結果.種目数
                        i = i + 1 : データ(i) = "         <td rowspan=""2"" style=""text-align: center;""  >" & 採点結果.種目(d).選手(s).種目得点.ToString("0") & "</td>"
                    Next d
                    i = i + 1 : データ(i) = "        </tr>"
        ...
        If 言語 = "E" Then
            i = i + 1 : データ(i) = "<h3> " & "  Detail Result </h3>"
        Else
            i = i + 1 : データ(i) = "<h3> " & "  詳細結果 </h3>"
        End If
