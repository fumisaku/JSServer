Public Class VAL得点詳細
    ...
    Public Sub CreateHTML(採点結果 As 採点結果_V2, LOCAL指定 As String)
        ...
        ElseIf 採点結果.採点方式 = "PDJ30B" Then
            採点方式名 = "AJS3.2J"
        ElseIf 採点結果.採点方式 = "VALQ25S" Then
            採点方式名 = "2025バルカーカップ採点方式"
        ...
        'バルカーでは種目毎ではなく、PCS毎に表示
        For p = 1 To 採点結果.マスタデータ.J_新審判設定.GetPCS数
            i = i + 1 : データ(i) = "	        <th>" & 採点結果.マスタデータ.J_新審判設定.PCS設定(p).PCS項目名 & "</th>"
        Next p

        '決勝ではオーディエンススコアを表示
        If 採点結果.ラウンド番号 = "400" Then
            i = i + 1 : データ(i) = "	        <th>オーディエンス</th>"
        End If
        ...
        For 順位 = 1 To 採点結果.出場選手数
            For s = 1 To 採点結果.出場選手数
                If 採点結果.総合順位表記(s) = 順位 Then
                    ...
                    For pp = 1 To 採点結果.マスタデータ.J_新審判設定.GetPCS数
                        PCS得点 = 採点結果.種目(1).選手結果(s).PCS得点(pp).PCS得点
                        i = i + 1 : データ(i) = "<td rowspan=""2"" class=""DanceScore"">" & PCS得点.ToString("0.000") & "</td>"
                    Next pp

                    'オーディエンス得点
                    'オーディエンスの結果
                    If 採点結果.ラウンド番号 = "400" Then
                        i = i + 1 : データ(i) = "<td rowspan=""2"" class=""DanceScore"">" & 採点結果.種目(1).選手結果(s).Other得点(2).結果得点.ToString("0.000") & "</td>"
                    End If
        ...
        For d = 1 To 採点結果.種目数
            ...
            If 採点結果.ラウンド番号 = "400" Then
                i = i + 1 : データ(i) = "   <th colspan=""" & 2 & """style=""text-align: center;"">得点</th>"
            Else
                i = i + 1 : データ(i) = "   <th colspan=""" & 1 & """style=""text-align: center;"">得点</th>"
            End If

            'i = i + 1 : データ(i) = "   <th colspan=""" & 減点数 & """style=""text-align: center;"">Reduction</th>"

            If 課題数 >= 1 Then
                i = i + 1 : データ(i) = "   <th colspan=""3"" style=""text-align: center;"">換算後</th>"
            ElseIf 採点結果.ラウンド番号 = "400" Then
                i = i + 1 : データ(i) = "   <th colspan=""3"" style=""text-align: center;"">換算後</th>"
            Else
                i = i + 1 : データ(i) = "   <th colspan=""2"" style=""text-align: center;"">換算後</th>"
            End If
            ...
            i = i + 1 : データ(i) = "      <th colspan=""1"" rowspan=""2"">" & "PCS" & "</th>"
            If 採点結果.ラウンド番号 = "400" Then
                i = i + 1 : データ(i) = "      <th colspan=""1"" rowspan=""2"">" & "オーディエンス" & "</th>"
            End If
            ...
            If 採点結果.ラウンド番号 = "400" Then
                i = i + 1 : データ(i) = "      <th rowspan=""2"">オーディエンス</th>"
            End If
                ...
                For j = 1 To 採点結果.種目(1).審判員数
                    ...
                    If 採点結果.種目(1).審判員(j).ジャッジタイプ <> "R" And 採点結果.種目(1).審判員(j).ジャッジタイプ <> "T" Then
                        ...
                        For p = 1 To PCS数
                            ...
                            ElseIf Strings.Left(採点結果.採点方式, 3) = "VAL" Then

                                If 採点結果.種目(d).選手結果(s).審判員結果(j).PCS素点(p).PCS採点対象FLAG = True Then


                                    If 採点結果.種目(d).選手結果(s).無効審判.無効判定(採点結果.種目(d).選手結果(s).背番号, 採点結果.種目(d).選手結果(s).審判員結果(j).ジャッジ記号) = 1 Then
                                        i = i + 1 : データ(i) = "<td class=""pcs"">-</td>"
                                        i = i + 1 : データ(i) = "<td class=""pcs2""></td>"

                                    Else

                                        If 採点結果.種目(d).選手結果(s).審判員結果(j).PCS素点(p).PCS無効FLAG = True Then  '上下カットされたとき

                                            i = i + 1 : データ(i) = "<td class=""pcs"">" & 採点結果.種目(d).選手結果(s).審判員結果(j).PCS素点(p).PCS素点.ToString("0.00") & "</td>"
                                            i = i + 1 : データ(i) = "<td class=""pcs2"">N/A</td>"
                        ...
                        If jj = 1 Then
                            ...
                            'PCS実得点
                            i = i + 1 : データ(i) = "<td rowspan=""" & 審判員数woR + 1 & """ class=""pcs"">" & 採点結果.種目(d).選手結果(s).Other得点(1).実得点.ToString("0.000") & "</td>"

                            'オーディエンスの結果
                            If 採点結果.ラウンド番号 = "400" Then
                                i = i + 1 : データ(i) = "<td rowspan=""" & 審判員数woR + 1 & """ class=""pcs"">" & 採点結果.種目(d).選手結果(s).Other得点(2).実得点.ToString("0.000") & "</td>"
                            End If
                            ...
                            'PCS合計点
                            i = i + 1 : データ(i) = "<td rowspan=""" & 審判員数woR + 1 & """ class=""pcs"">" & 採点結果.種目(d).選手結果(s).Other得点(1).結果得点.ToString("0.000") & "</td>"

                            If 採点結果.ラウンド番号 = "400" Then
                                'オーディエンス
                                i = i + 1 : データ(i) = "<td rowspan=""" & 審判員数woR + 1 & """ class=""pcs"">" & 採点結果.種目(d).選手結果(s).Other得点(2).結果得点.ToString("0.000") & "</td>"
                            End If
    ...
    Private Function UP検索(背番号 As String, 採点結果 As 採点結果_V2) As String
        ...
        For i = 1 To 採点結果.マスタデータ.E_ヒート表マスタ.登録済みレコード数
            ...
            If 採点結果.マスタデータ.E_ヒート表マスタ.リスト(i).背番号 = 背番号 Then
                rc = "UP"
