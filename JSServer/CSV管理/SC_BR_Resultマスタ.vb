Public Class SC_BR_Resultマスタ

    Public リスト(100) As SC_BR_Result

    Public 登録済みレコード数 As Integer
    Private ReadOnly filepath As String

    Const File頭文字列 = "SC_BR_Result"
    ...
    ''' 
    Public Function 登録(データ As SC_BR_Result) As Integer
        ...
        Try
            ...
            'ヘッダーを書き出し
            sw.WriteLine("カテゴリ番号,カテゴリ名,区分番号,ラウンド番号,勝者背番号,勝者リーダー名,勝者パートナー名,勝者所属,勝者点数,勝者WIN数,敗者点数,敗者WIN数")
    ...
    '''
    Private Function カンマ区切り(BR As SC_BR_Result)
        ...
        line = line & "," & BR.ラウンド番号
        line = line & "," & BR.勝者背番号
        line = line & "," & BR.勝者リーダー名
        line = line & "," & BR.勝者パートナー名
        line = line & "," & BR.勝者所属
        line = line & "," & BR.勝者点数
        line = line & "," & BR.勝者WIN数
        line = line & "," & BR.敗者点数
    ...
    Private Sub Addデータ(データ As String)
        ...
        If UBound(リスト) < No Then
            MsgBox("SC_BR_Result.csv の行数がMAXを超えました。クリアするかデータを削除してください。")
            ReDim Preserve リスト(No)
        End If


        If UBound(arBuffer) >= 6 Then

            リスト(No) = New SC_BR_Result

            If UBound(arBuffer) < 11 Then
                MsgBox("SC_BR_Result.csv の" & No & "行目のデータが異常です。")
            ...
            Else
                ...
                リスト(No).勝者背番号 = arBuffer(4)
                リスト(No).勝者リーダー名 = arBuffer(5)
                リスト(No).勝者パートナー名 = arBuffer(6)
                リスト(No).勝者所属 = arBuffer(7)
                リスト(No).勝者点数 = arBuffer(8)
                リスト(No).勝者WIN数 = arBuffer(9)
                リスト(No).敗者点数 = arBuffer(10)
                リスト(No).敗者WIN数 = arBuffer(11)
