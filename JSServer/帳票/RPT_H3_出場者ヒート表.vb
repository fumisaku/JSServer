Public Class RPT_H3_出場者ヒート表
    ...
    Public Sub 印刷(ByVal 区分番号 As String, ByVal ラウンド番号 As String, ByVal マスタデータ As マスタデータ, ByVal 配布先() As String)
        ...
        Dim RPT As RPT_R3_縦マスタ01
        RPT = New RPT_R3_縦マスタ01(マスタデータ.Z_システム設定.言語)
        ...
        For ss = 1 To マスタデータ.C_ラウンドマスタ.出場組数(区分番号, ラウンド番号)
            ...
            Parm.ヒートText(行数) = CStr(Min背番号).PadLeft(8) & "  "
            For d = 1 To マスタデータ.D_種目マスタ.Get_種目数(区分番号, ラウンド番号, 種目記号リスト)
    ...
    Private Sub 印刷実行(RPT As RPT_R3_縦マスタ01, Parm As RPT_Parm_H2, 配布先() As String）
